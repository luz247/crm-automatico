...
export const useCrmStore = () => {
  ...
  const setInfoAll = ({ company }) => {
    try {
      console.log(company, "soy el numero de carteraaaa");
  ...
  const bacgroundWallet = ({company}) => {
    console.log("souel backgro", {company});
    try {
      dispatch(onPaintWallet({company}));
    } catch (error) {
      console.log(error);
    }
  };
