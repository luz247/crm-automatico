...
export const FromCrm = () => {
  ...
  const telefono = params.get("phone_number");
  const rutEje = params.get("user");
  ...
  const think = async (formattedValues, resetForm, status) => {
    ...
    console.log('soy el que pone a dipo ',{ user: user.rut, status })
  ...
  const formSubmit = async (values, { resetForm }) => {
    ...
    if (!activeData || activeData.length === 0) {
      Swal.fire("Rut no exite en base de datos Validar si ya pago!");
    } else {
      try {
    ...
    >
      {({ handleSubmit, setFieldValue, values }) => (
        ...
        >
          ...
          <Row style={{ display: "flex", marginBottom: "16px",  alignContent:'center',justifyContent:'space-between'}}>
            <Form.Item label="Fono Getion" style={{ marginBottom: 0 }}>
              
              <div style={{ fontSize: '15px', fontWeight: '700', paddingLeft: '10px' }}>
                {
                  telefono
                }
              </div>
